...
typedef enum {SOURIS, LION, ELEPHANT} typePiece;
...
struct plateau {
    int hauteur;
    int largeur;
    char contenu[hauteur][largeur];
};
	


struct piece initialiserPieces2(piece p, typePiece ty, couleurPiece co, caracterePiece cp){
    p->type = ty;
    p->couleur = co;
    p->caractere = cp;
...
void initialiserPieces() {
    piece sourisNoir1;
    initialiserPieces2(sourisNoir1, SOURIS, NOIR, S);

    piece sourisNoir2;
    initialiserPieces2(sourisNoir2, SOURIS, NOIR, S);
