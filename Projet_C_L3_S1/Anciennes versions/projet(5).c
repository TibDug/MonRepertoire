  ...
  void afficherPlateau (struct plateau *plat){
      for (int i = 0; i < plat ->  hauteur; i++){
        for (int j = 0; j < plat ->  largeur; j++){
    	    ...
    	    case VIDE : if(plat -> contenu[i][j].pointEau == 0) printf("."); else printf("*"); break;
  ...
  */
  ...
  void changementJoueur (struct joueur *j1, struct joueur *j2) {
    if (j1->tour == 1) {
      j1->tour = 0;
      j2->tour = 1;
    }
  ...
  void partie(struct plateau *plat){
    ...
    do{
      ...
      afficherPlateau(plat);
      changementJoueur(joueurNoir, joueurBlanc);
